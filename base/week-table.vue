...
<template>
  <div :class="classes">
    <div :class="[prefixCls + '-header']">
      <span v-if="showWeekNumbers" style="width: 35px;">周次</span>
      <span v-for="day in headerDays" :key="day">{{day}}</span>
    </div>
    <span :class="getCellCls(cell)" v-for="(cell, i) in cells" :key="String(cell.date) + i" @click="handleClick(cell, $event)"
    ...
    export default {
        ...
        computed: {
            ...
            calendar() {
                ...
                const lang = this.$config.lang.toLowerCase()
                return new jsCalendar.Generator({onlyDays: !this.showWeekNumbers, weekStart: weekStartDay, lang: lang});
            },
            headerDays() {
                const weekStartDay = Number(this.t('i.datepicker.weekStartDay'));
                ...
                return translatedDays.splice(weekStartDay, 7 - weekStartDay).concat(translatedDays.splice(0, weekStartDay));
            },
            cells() {
                ...
                return this.calendar(tableYear, tableMonth, null, (cell) => {
